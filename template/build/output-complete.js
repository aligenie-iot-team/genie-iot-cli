...
class OutputQrcode {
  ...
  apply(compiler) {
    const executeOpen = once(() => {
      setTimeout(() => {
        const url = 'https://ju.taobao.com/m/jusp/alone/iot-bind-dev/mtp.htm#/list/';
        qrcode.generate(url);
        console.log('1. 打开天猫精灵app');
        console.log('2. 扫码获取用户身份信息AuthInfo');
        console.log(`3. 在浏览器控制台输入localStorage.setItem('__authInfo', {...});localStorage.setItem('__isDebug', 'true')`);
